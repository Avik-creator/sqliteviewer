...
export default function SQLiteViewer() {
  ...
  const getTables = (db: any) => {
    ...
    return tables.values.map((table: any[]) => {
      ...
      const columns = db.exec(`PRAGMA table_info(${tableName})`)[0].values.map((col: any[]) => col[1]);
      const rowCount = db.exec(`SELECT COUNT(*) FROM ${tableName}`)[0].values[0][0];
      ...
      <main className="max-w-[1400px] mx-auto">
        ...
        {!db ? (
          ...
          <div className="space-y-4">
            ...
            <Card className="w-full bg-[#12141a] border-gray-800">
              <CardHeader>
                <CardTitle className="text-base font-normal">Execute Query</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <Textarea 
